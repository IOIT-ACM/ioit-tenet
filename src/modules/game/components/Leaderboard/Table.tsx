...
export const LeaderboardTable: FC<LeaderboardTableProps> = ({
  ...
  const evenRow = 'bg-slate-700 bg-opacity-30';
  const oddRow = 'bg-slate-800 bg-opacity-15';
  const headerCell =
    'px-2 py-2 text-center font-medium text-slate-300 uppercase tracking-wider text-xs md:text-sm md:px-3 md:py-3 lg:text-lg lg:px-6 lg:py-3';
  const rowCell =
    'px-2 py-2 whitespace-nowrap text-sm text-center text-slate-300 text-xs md:text-sm md:px-3 md:py-3 lg:text-lg lg:px-6 lg:py-4';
  ...
  return (
    <div className={'border-corners relative'}>
            ...
            {data.map((item, index) => {
              ...
              return (
                <tr key={item.id} className={rowClass}>
                  <td className={`${rowCell} font-bold`}>
                    #{index < 50 ? index + 1 : '50+'}
                  </td>
      ...
      {newUser && (
        <NewUserPopup item={newUser} key={`new-user-${newUser.id}`} />
      )}
    </div>
  );
};
